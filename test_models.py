...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    ...
    data_loader = torch.utils.data.DataLoader(
            PANDataSet(root_path, test_file if test_file is not None else val_list, num_segments=this_test_segments,
                       ...
                       ]), dense_sample=args.dense_sample, twice_sample=args.twice_sample, is_lmdb=args.lmdb),
...
if args.csv_file is not None:
    ...
    with open('sth_category.txt') as f:
        categories = f.readlines()
    categories = sorted([f.strip() for f in categories])
...
name_list = [x.strip().split()[0] for x in open(val_list)]
...
if set(['PA', 'RGB']) == set(modality_list):
    modality = 'Full'
elif set(['PA', 'RGB', 'Lite']) == set(modality_list):
    modality = 'En'

np.savez("_".join([args.dataset, modality, str(top1.avg)]), scores=reorder_output, labels=reorder_label)
